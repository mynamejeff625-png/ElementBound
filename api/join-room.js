  ...
  return async function joinRoom(req,res){
    ...
    try{
      ...
      const result=await db.runTransaction(async transaction=>{
        ...
        transaction.update(roomRef,{status:'ACTIVE',players:activePlayers,deckSelections:[creatorSelection,joinSelection],state,updatedAt:Date.now()});
        for(let seat=0;seat<2;seat++)transaction.set(roomRef.collection('views').doc(activePlayers[seat]),{status:'ACTIVE',roomId,seat,state:buildPlayerView(state,seat),updatedAt:Date.now()});
