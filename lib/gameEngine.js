(function(root,factory){
  ...
  function stateChangeEvents(before,after,move){
    const events=[],oldCards=cardMap(before),newCards=cardMap(after);
    for(let seat=0;seat<2;seat++){
      const oldSide=before.p[seat],newSide=after.p[seat];
      if(newSide.vit<oldSide.vit){
        const amount=oldSide.vit-newSide.vit;
        events.push(event('BENDER_DAMAGE',{seat,targetType:'BENDER',amount,remainingVitality:newSide.vit},`${newSide.name} takes ${amount} damage`));
      }
      const oldMarks=new Set(oldSide.marks||[]);
      for(const status of newSide.marks||[])if(!oldMarks.has(status))events.push(event('STATUS_APPLIED',{seat,targetType:'BENDER',status},`${status} applied to ${newSide.name}`));
    }
    for(const [id,next] of newCards){
      ...
      if((next.card.h??0)<(prior.card.h??0)&&move.type!=='END_TURN'){
        ...
        events.push(event('DAMAGE',{seat:next.seat,targetType:'CARD',targetId:id,targetName:next.card.n,amount,armorAbsorbed,remainingHp},`${next.card.n} takes ${amount} damage${armorAbsorbed?` · Armor blocks ${armorAbsorbed}`:''} · ${remainingHp} HP remains`));
      }else if((prior.card.armor||0)>(next.card.armor||0)&&move.type==='ATTACK'){
        const armorAbsorbed=(prior.card.armor||0)-(next.card.armor||0);
        events.push(event('DAMAGE',{seat:next.seat,targetType:'CARD',targetId:id,targetName:next.card.n,amount:0,armorAbsorbed,remainingHp:next.card.h},`${next.card.n} takes 0 damage · Armor blocks ${armorAbsorbed} · ${next.card.h} HP remains`));
      }
      const oldMarks=new Set(prior.card.marks||[]);
      for(const status of next.card.marks||[])if(!oldMarks.has(status))events.push(event('STATUS_APPLIED',{seat:next.seat,targetType:'CARD',targetId:id,targetName:next.card.n,status},`${status} applied to ${next.card.n}`));
      if(prior.zone==='FIELD'&&next.zone==='wake')events.push(event('DESTROYED',{seat:next.seat,cardId:id,cardName:next.card.n},`${next.card.n} destroyed`));
    }
    return events;
  }
  ...
  function validateAndApplyMove(state,move){
    ...
    if(move.type==='PLAY_CARD'&&played){
      ...
      if(drawn)events.push(event('DRAW',{actor:nextSeat,actorName:next.p[nextSeat].name,cardId:drawn.id,cardName:drawn.n,privateTo:nextSeat},`${next.p[nextSeat].name} draws ${drawn.n}`));
    }
