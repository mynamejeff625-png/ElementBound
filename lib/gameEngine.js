(function(root,factory){
  ...
  function responseTargets(element,defender,target,phase,round){
    if(!target)return [];
    ...
    if(element==='EARTH')return defender.slots.filter(card=>card&&card.armorGainRound!==round&&(card.armor||0)<3);
  ...
  function responseOptions(state,defenderSeat,pending,phase){
    ...
    for(const element of responseElements(defender)){
      const targets=responseTargets(element,defender,target,phase,state.turn);if(!targets.length)continue;
  ...
  function resolveAttack(state,owner,attacker,target){
    ...
    if(!target){
      ...
      const dealt=soakedPower(attacker,Math.max(0,(attacker.a||0)+attackBonus(state,owner,attacker,enemy)));
      enemy.vit-=dealt;return {dealt};
    }
    ravager(side,attacker,target);quickBefore(enemy,target);
    if(!enemy.slots.includes(target))return {dealt:0};
  ...
  function openResponseWindow(state,attackInfo,timing,now){
    ...
    if(!Number.isFinite(now))return {error:'MISSING_SERVER_TIME'};
    pending.deadline=now+RESPONSE_WINDOW_MS;
  ...
  function continuePendingAttack(state,pending,events,now){
    ...
    const before=clone(state),outcome=resolveAttack(state,pending.attackerSeat,attacker,target);
    events.push(...stateChangeEvents(before,state,{type:'ATTACK',payload:{targetId:target.id}}));
    if(outcome.dealt>0){const after=openResponseWindow(state,pending,'AFTER',now);if(after)events.push(responseOfferEvent(after))}
  }
  ...
  function validateAndApplyMove(state,move,options={}){
    ...
    if(state.pendingResponse){
      if(!Number.isFinite(now))return reject(state,'MISSING_SERVER_TIME');
      ...
      if(expired){
        ...
        if(move.type==='RESOLVE_EXPIRED')return reject(state,'RESPONSE_NOT_EXPIRED',{deadline:pending.deadline,serverNow:now,remainingMs:pending.deadline-now});
      ...
      return {ok:true,error:null,autoResolved:expired,state:next,events:events.map(item=>({turn:before.turn,...item}))};
    }
    ...
    if(move.type==='PLAY_CARD')error=playCard(next,move);
      ...
      if(!attacker)error='ATTACKER_NOT_ON_FIELD';
        ...
        if(pending?.error)error=pending.error;
        else if(pending)events.push(responseOfferEvent(pending));
        else{
          const damageBefore=clone(next),outcome=resolveAttack(next,move.actor,attacker,target),amount=outcome.dealt;
          ...
          if(amount>0){const after=openResponseWindow(next,info,'AFTER',now);if(after?.error)error=after.error;else if(after)events.push(responseOfferEvent(after))}
        }
