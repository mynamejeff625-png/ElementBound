...
assert(versionNodes.every(node=>node.textContent.includes(releaseVersion)));
assert(versionContext.document.title.includes(releaseVersion));
assert(html.indexOf('js/version.js')<html.indexOf('js/game.js'));
assert(!/(?:Alpha|EB-) 0\.8\.(?:39|44|47|50|51|53)/.test(html));
console.log('Central release metadata checks passed');
