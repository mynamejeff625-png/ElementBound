...
(async()=>{
  ...
  check(/modal\('Incoming attack',buttons,\{dismissible:false\}\)/.test(game)&&/onClick:defending\?ebMpOpenResponsePrompt/.test(game),'response prompt has no dismiss action and its status banner can reopen it');
  const modalSource=game.match(/function modal\([^\n]+/)[0],classes=new Set(['hide']),elements={mt:{},mb:{innerHTML:'',appendChild(){}},modalDismiss:{style:{}},mw:{offsetWidth:1,classList:{contains:value=>classes.has(value),add:(...values)=>values.forEach(value=>classes.add(value)),remove:(...values)=>values.forEach(value=>classes.delete(value))}}};
  ...
  const early=await client.submit(action('RESOLVE_EXPIRED',0,1)),raced=await client.submit(action('RESOLVE_EXPIRED',0,1)),eventual=await client.submit(action('RESOLVE_EXPIRED',0,1));
  ...
  check(!raced.ok&&raced.error==='REVISION_MISMATCH'&&!messages.some(message=>message.kind==='error'),'a raced expiry revision mismatch does not show the generic match-changed banner');
