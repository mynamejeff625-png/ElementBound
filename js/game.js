...
EARTH:[['Stone Initiate',1,1,3,'On summon: give another friendly Manifestation 1 Armor, max 3, until your next turn.','Redistribute temporary Earth defense onto an established ally; unused Armor expires at the start of your next turn.'],['Earthen Guard',2,1,5,'Guard.','Protects your Bender while it remains on the field, even after it attacks.'],['Boulder Ram',3,3,5,'If it has Armor when it attacks, deal +1 damage.','Converts temporary Earth defense into pressure before that Armor is consumed or expires.']],
...
AIR:[['Breeze Disciple',1,1,3,'On summon: give another friendly Manifestation 2 Momentum, max 3, until your next turn.','Put temporary Momentum onto an established ally such as Sky Raptor to unlock Air’s pressure payoff.'],['Gale Scout',2,2,3,'After Crosswind swaps two enemies, this gains 1 Momentum on its next attack that turn.','Use Crosswind first, then attack to gain temporary Momentum.'],['Sky Raptor',3,3,3,'While it has Momentum, it may attack the Bender ignoring Guard.','Air pressure payoff: temporary Momentum lets Sky Raptor choose the rival Bender even while Guard is active.']]};
const RESPONSES={
...
EARTH:{n:'Stonewall',c:2,text:'When one of your Manifestations is attacked, give a friendly Manifestation +1 Armor, max 3, until your next turn.',tip:'Temporary protection; unused Armor expires at the start of your next turn.'},
...
EARTH:['Fortify','Give a friendly Manifestation 1 Armor, max 3, until your next turn.','Use before an enemy attack or to enable temporary Armor payoffs.',2],
...
AIR:['Crosswind','Swap the positions of two enemy Manifestations. If fewer than two are present, give a friendly Manifestation 1 Momentum, max 3, until your next turn.','Disrupts positioning or builds temporary Air pressure.',1]};
const HYBRID_CARDS={
MAGMA:[
 {n:'Molten Channel',c:2,type:'TECHNIQUE',role:'SETUP',text:'Choose: give a friendly Manifestation 1 Armor until your next turn, or apply Burning to an enemy Manifestation damaged this turn. Resonance — choose both.',tip:'Flexible Magma setup; unused Armor expires at the start of your next turn.'},
 ...
 {n:'Pressure Forge',c:3,type:'TECHNIQUE',role:'PAYOFF',text:'Give a friendly Manifestation 1 Armor until your next turn. Resonance — if an enemy is Burning, deal 1 damage to it.',tip:'Temporary defensive value with a controlled Resonance payoff.'},
...
STORM:[
 {n:'Crosswind Spark',c:1,type:'TECHNIQUE',role:'SETUP',text:'Move a friendly Manifestation to an empty friendly slot and give it 1 Momentum, max 3, until your next turn. Resonance — it also becomes Charged.',tip:'Repositions and prepares temporary Storm payoffs.'},
...
'Armor':'Temporary damage protection attached to a specific Manifestation, up to 3. Each Armor prevents 1 damage and is consumed. Any unused Armor expires at the start of that Manifestation owner’s next turn.',
...
'Momentum':'Air’s temporary stacking resource, up to 3. Multiple grants can stack during the round, and each Momentum gives that Manifestation +1 ATK. All Momentum expires at the start of that Manifestation owner’s next turn. Sky Raptor can ignore Guard while it currently has Momentum.',
...
function expireRoundEffects(m){if(!m)return;let stacks=momentumStacks(m);if(stacks){m.a=Math.max(0,(Number(m.a)||0)-stacks);m.momentum=0;m.marks=(m.marks||[]).filter(x=>x!=='Momentum')}m.armor=0}
...
function turnStart(){
 ...
 let p=current();p.slots.filter(Boolean).forEach(m=>{expireRoundEffects(m);m.quick=null;m.turnFlags={}});p.turnState=freshTurnState(p.el);p.maxE=Math.min(7,2+Math.floor((G.turn-1)));p.e=p.maxE;p.slots.filter(Boolean).forEach(m=>{m.ready=true;m.sick=false});draw(p,true);G.chain=0;bump(`${p.name} turn begins`)}
...
const EB_BALANCE_PATCH='BALANCE-PASS-0.8.60';
const EB_BALANCE=(()=>{
 ...
 function expireRoundEffectsSim(x){if(!x)return;let stacks=momentum(x);if(stacks){x.a=Math.max(0,(Number(x.a)||0)-stacks);x.momentum=0;unmark(x,'Momentum')}x.armor=0}
 ...
 return Object.freeze({DECKS:[...DECKS],simulate,aggregate,matrix,paired,selfTest,version:'LAB-XVII-BALANCE-PASS'});
})();
