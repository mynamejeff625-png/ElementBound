...
let attackers=p.slots.filter(x=>x&&x.ready&&!x.sick);if(diff==='Easy')attackers=attackers.slice(0,1);
 ...
 function nextAttack(i){if(G.winner||i>=attackers.length)return finishAI();let m=attackers[i];if(!p.slots.includes(m)||!m.ready||m.sick)return nextAttack(i+1);m.ready=false;let guards=activeGuards(e),targets=e.slots.filter(Boolean),t=null;let canBypass=guards.length>0&&attackBypassesGuard(m,p);if(canBypass&&diff==='Difficult')t=null;else if(targets.length){t=diff==='Easy'?targets[Math.floor(Math.random()*targets.length)]:ebPickAITarget(m,targets,Math.random)}
  if(!t){let power=ebSoakedAttackPower(m,Math.max(0,m.a+elementalAttackBonus(m,e,p))); e.vit-=power;ebQueueFx({kind:'benderHit',side:0,damage:power,el:m.el,label:m.n});add(`Rival ${m.n} hits Bender for ${power}`);winCheck();return nextAttack(i+1)}
...
function runEBVerification(){
 ...
 EB_DEV_LAST_REPORT=[`ELEMENT BOUND Alpha 0.8.54 CANDIDATE B SYSTEM CHECK`,`${pass}/${total} PASSING`,...results.map(r=>`${r.ok?'PASS':'FAIL'} [${r.group}] ${r.name}${r.detail?' — '+r.detail:''}`)].join('\n');
 return results;
}
...
const EB_BALANCE_PATCH='CANDIDATE-B-WATER'; // 0.8.54: Soaked weakens the marked unit's next direct attack by 2.
const EB_BALANCE=(()=>{
...
const EB_BALANCE_RULESET='EB-RULES-0.8.54-CANDIDATE-B';
