...
function ebAttackCore(att,t){hideModal();let defending=foe(),owner=me();
 ...
 att.ready=false;if(t){applyQuickBeforeAttack(foe(),t);let q=t.quick&&t.quick.kind==='REDUCE'?t.quick.value:0;if(q)t.quick=null;let power=ebSoakedAttackPower(att,Math.max(0,att.a+elementalAttackBonus(att,t,me())-q));
...
function runEBVerification(){
 ...
 try{
  ...
  test('WATER','Soaked weakens and consumes on the marked unit next direct attack',()=>{let x=ebSoakedAttackPower.toString();ebAssert(x.includes("indexOf('Soaked')")&&x.includes('power-2')&&x.includes("x!=='Soaked'"))});
 ...
 summary.innerHTML=`<b>SYSTEM HEALTH: ${pass} / ${total} PASSING</b><div class="small">${pct}% verified · ${total-pass} issue${total-pass===1?'':'s'} detected</div><div class="devBar"><span class="devBarFill" style="width:${pct}%"></span></div>`;
 ...
 EB_DEV_LAST_REPORT=[`ELEMENT BOUND Alpha 0.8.54 SYSTEM CHECK`,`${pass}/${total} PASSING`,...results.map(r=>`${r.ok?'PASS':'FAIL'} [${r.group}] ${r.name}${r.detail?' — '+r.detail:''}`)].join('\n');
 return results;
}
...
/* ================================================================
   ...
   ================================================================ */
const EB_BUILD={alpha:'0.8.54',engine:'EB-1.8.54'};
const EB_BALANCE_PATCH='WATER-SOAKED-MITIGATION'; // 0.8.54: Soaked weakens the marked unit's next direct attack by 2.
const EB_BALANCE=(()=>{
...
const EB_BALANCE_RULESET='EB-RULES-0.8.54';
const EB_BALANCE_LAB_VERSION='0.8.54';
...
function ebBalanceRenderResults(rep){let games=rep.rows.reduce((n,r)=>n+r.games,0),stalls=rep.rows.reduce((n,r)=>n+r.stalls,0),turns=rep.rows.reduce((n,r)=>n+r.turns,0),actions=rep.rows.reduce((n,r)=>n+r.actions,0),side=[0,0],reasons={},cards={},effects={};rep.rows.forEach(r=>{side[0]+=r.wins[0];side[1]+=r.wins[1];for(const[k,v]of Object.entries(r.reasons))reasons[k]=(reasons[k]||0)+v;for(const[k,v]of Object.entries(r.cards))cards[k]=(cards[k]||0)+v;for(const[k,v]of Object.entries(r.effects))effects[k]=(effects[k]||0)+v});let status=rep.cancelled?'CANCELLED — PARTIAL RESULTS':'COMPLETE';document.getElementById('blProgress').innerHTML=`<b class="${rep.cancelled?'blWarn':'blGood'}">${status}</b><div class="small">${games.toLocaleString()} games processed in ${(rep.elapsedMs/1000).toFixed(1)}s.</div><div class="devBar"><span class="devBarFill" style="width:${Math.round(games/rep.totalPlanned*100)}%"></span></div>`;document.getElementById('blSummary').innerHTML=`<b>GLOBAL SUMMARY</b><div class="blStatGrid"><div class="blStat"><small>Side A wins</small><b>${ebBLPct(side[0],games)}%</b></div><div class="blStat"><small>Side B wins</small><b>${ebBLPct(side[1],games)}%</b></div><div class="blStat"><small>Avg turns</small><b>${games?(turns/games).toFixed(2):'0'}</b></div><div class="blStat"><small>Avg actions</small><b>${games?(actions/games).toFixed(2):'0'}</b></div><div class="blStat"><small>Stalls</small><b>${stalls}</b></div><div class="blStat"><small>Completed</small><b>${games.toLocaleString()}</b></div></div><div class="blUtil"><b>Win reasons:</b> ${ebBLTop(reasons,8).map(x=>`${x[0]} ${x[1]}`).join(' · ')||'none'}<br><b>Top effects:</b> ${ebBLTop(effects).map(x=>`${x[0]} ${x[1]}`).join(' · ')||'none'}</div>`;ebBalanceRenderMatrix(rep);document.getElementById('blCopy').disabled=false}
...
async function ebTempoLabRun(){
 ...
 try{box.innerHTML='<b>RUNNING TEMPO LAB VII…</b><div id="blTempoProgress" class="small">Preparing finalist calibration…</div><div class="devBar"><span class="devBarFill" id="blTempoBar"></span></div><div class="small">Paired mirror seeds · five simulator-only finalists · role/seat pairing · sequencing · lifecycle · first-removal correlation.</div>';
...
function ebBalanceClear(){if(EB_BL_RUNNING)return;EB_BL_LAST=null;document.getElementById('blSummary').innerHTML='<b>NO REPORT YET</b><div class="small">Run the lab to generate matchup, turn-length, utilization and stall telemetry.</div>';document.getElementById('blProgress').innerHTML='<b>READY</b><div class="small">No simulation is running.</div><div class="devBar"><span class="devBarFill" id="blBar"></span></div>';document.getElementById('blMatrixWrap').classList.add('blHidden');document.getElementById('blDetail').classList.add('blHidden');document.getElementById('blCopy').disabled=true}
