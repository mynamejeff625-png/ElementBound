...
EARTH:[['Stone Initiate',1,1,3,'On summon: give another friendly Manifestation 1 Armor, max 3, until round end.','Redistribute temporary Earth defense onto an established ally; unused Armor expires after both Benders have acted.'],['Earthen Guard',2,1,5,'Guard.','Protects your Bender while it remains on the field, even after it attacks.'],['Boulder Ram',3,3,5,'If it has Armor when it attacks, deal +1 damage.','Converts temporary Earth defense into pressure before that Armor is consumed or expires.']],
...
EARTH:{n:'Stonewall',c:2,text:'When one of your Manifestations is attacked, give a friendly Manifestation +1 Armor, max 3, until round end.',tip:'Temporary protection; unused Armor expires after both Benders have acted.'},
...
EARTH:['Fortify','Give a friendly Manifestation 1 Armor, max 3, until round end.','Use before the round ends to enable temporary Armor payoffs.',2],
...
const HYBRID_CARDS={
MAGMA:[
 {n:'Molten Channel',c:2,type:'TECHNIQUE',role:'SETUP',text:'Choose: give a friendly Manifestation 1 Armor until round end, or apply Burning to an enemy Manifestation damaged this turn. Resonance — choose both.',tip:'Flexible Magma setup; unused Armor expires after both Benders have acted.'},
 ...
 {n:'Pressure Forge',c:3,type:'TECHNIQUE',role:'PAYOFF',text:'Give a friendly Manifestation 1 Armor until round end. Resonance — if an enemy is Burning, deal 1 damage to it.',tip:'Temporary defensive value with a controlled Resonance payoff.'},
...
'Armor':'Temporary damage protection attached to a specific Manifestation, up to 3. Each Armor prevents 1 damage and is consumed. Any unused Armor expires at round end, after both Benders have acted.',
...
function expireRoundEffects(m){if(!m)return;let stacks=momentumStacks(m);if(stacks){m.a=Math.max(0,(Number(m.a)||0)-stacks);m.momentum=0}m.armor=0;m.marks=(m.marks||[]).filter(x=>x!=='Momentum'&&x!=='Charged')}
...
const EB_BALANCE_PATCH='ROUND-END-DECAY-0.8.61';
const EB_BALANCE=(()=>{
 ...
 function expireRoundEffectsSim(x){if(!x)return;let stacks=momentum(x);if(stacks){x.a=Math.max(0,(Number(x.a)||0)-stacks);x.momentum=0}x.armor=0;unmark(x,'Momentum');unmark(x,'Charged')}
 ...
 return Object.freeze({DECKS:[...DECKS],simulate,aggregate,matrix,paired,selfTest,version:'LAB-XVIII-ROUND-END-DECAY'});
})();
