...
function resolveHybridTechnique(p,e,c,isAI=false,target=null){let r=resonant(p),friend=(target&&target.friend)||p.slots.find(Boolean),enemy=(target&&target.enemy)||e.slots.find(m=>m&&wasDamagedThisTurn(m));
 if(c.el==='MAGMA'){if(c.n==='Molten Channel'){let mode=target&&target.magmaMode;if(!mode)mode=r?'BOTH':(enemy?'BURNING':'ARMOR');if((mode==='ARMOR'||mode==='BOTH')&&friend){friend.armor=Math.min(3,(friend.armor||0)+1);add(`${c.n}: ${friend.n} gains Armor`)}if((mode==='BURNING'||mode==='BOTH')&&enemy&&wasDamagedThisTurn(enemy)){addMark(enemy,'Burning');add(`${c.n}: ${enemy.n} was damaged this turn → Burning`)}}else if(c.n==='Pressure Forge'){if(friend)friend.armor=Math.min(3,(friend.armor||0)+1);if(r){let t=e.slots.find(x=>x&&(x.marks||[]).includes('Burning'));if(t){hit(t,1);death(e);add(`${c.n}: Resonance deals 1`)}}}else if(c.n==='Eruption Guard'&&friend){friend.quick={kind:'REDUCE',value:2,ownerTurn:G.turn};add(`${c.n}: protection armed`)}}
...
const EB_BALANCE=(()=>{
 ...
 function playTechnique(st,owner,c){let p=st.p[owner],e=st.p[1-owner],res=!!p.turnState.resonance.active;
   ...
   else if(c.el==='MAGMA'){let friend=p.slots.filter(Boolean)[0],damaged=e.slots.find(x=>x&&simDamagedThisTurn(st,owner,x));if(c.n==='Molten Channel'){let burn=damaged&&(!(damaged.marks||[]).includes('Burning')||res);if((res||!burn)&&friend){friend.armor=Math.min(3,(friend.armor||0)+1);metric(st,'effect','Armor');metric(st,'effect','Molten Channel Armor')}if(burn){mark(damaged,'Burning');metric(st,'effect','Burning applied');metric(st,'effect','Molten Channel Burning')}}else if(c.n==='Pressure Forge'){if(friend){friend.armor=Math.min(3,(friend.armor||0)+1);metric(st,'effect','Armor');metric(st,'effect','Pressure Forge Armor')}if(res){let q=e.slots.find(x=>x&&(x.marks||[]).includes('Burning'));if(q){dealBody(st,owner,q,1,c);metric(st,'effect','Pressure Forge damage')}}}else if(c.n==='Eruption Guard'&&friend){friend.quick={kind:'REDUCE',value:2};metric(st,'effect','Eruption Guard armed')}}
