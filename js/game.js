...
LIGHTNING:[['Spark Runner',1,2,2,'If this is your second card played this turn, gain +1 Chain.','Chain accelerator: play it second to jump directly from Chain 2 to Chain 3.'],['Arc Runner',2,2,3,'At Chain 2+, this gets +2 ATK this turn.','Rewards playing cards in sequence before attacking.'],['Volt Lynx',3,4,3,'At Chain 3+, its first attack each turn deals +1 damage.','Fragile but explosive combo payoff.']],
AIR:[['Breeze Disciple',1,1,3,'On summon: give another friendly Manifestation 2 Momentum, max 3.','Put Momentum onto an established ally such as Sky Raptor to unlock Air’s pressure payoff.'],['Gale Scout',2,2,3,'After Crosswind swaps two enemies, this gets +2 ATK on its next attack that turn.','Use Crosswind first, then attack to exploit the opening.'],['Sky Raptor',3,3,3,'If it has Momentum, it may attack the Bender ignoring Guard.','Air pressure payoff: give Sky Raptor Momentum, then it can legally choose the rival Bender even while Guard is active.']]};
const RESPONSES={
...
EARTH:['Fortify','Give a friendly Manifestation 1 Armor, max 3.','Use before an enemy attack or to enable Armor payoffs.',2],
...
function elementalAttackBonus(att,target,p){
 ...
 if(att.el==='LIGHTNING'&&att.n==='Arc Runner'&&G.chain>=2){bonus+=2;add(`LIGHTNING · Arc Runner Chain ${G.chain} → +2 ATK`)}
...
function runEBVerification(){
 ...
 try{
  ...
  test('LIGHTNING','Arc Runner gets +2 at Chain 2',()=>{let p=ebTestPlayer('LIGHTNING'),e=ebTestPlayer();G={p:[p,e],active:0,chain:2,logs:[]};ebAssert(elementalAttackBonus(ebTestCard('LIGHTNING','Arc Runner'),e,p)===2)});
  ...
  test('LIGHTNING','Arc Runner + Chain 2 + Charged stacks to +3',()=>{let p=ebTestPlayer('LIGHTNING'),e=ebTestPlayer();e.marks=['Charged'];G={p:[p,e],active:0,chain:2,logs:[]};ebAssert(elementalAttackBonus(ebTestCard('LIGHTNING','Arc Runner'),e,p)===3)});
...
const EB_BALANCE_PATCH='BALANCE-PASS-0.8.57';
const EB_BALANCE=(()=>{
 ...
 function attackBonus(st,owner,att,target){let p=st.p[owner],bonus=0;if(att.el==='FIRE'&&att.n==='Flare Hawk'&&(target.marks||[]).includes('Burning'))bonus++;
   ...
   if(att.el==='LIGHTNING'&&att.n==='Arc Runner'&&p.chain>=2)bonus+=2;
 ...
 return Object.freeze({DECKS:[...DECKS],simulate,aggregate,matrix,paired,selfTest,version:'LAB-XIV-BALANCE-PASS'});
})();
